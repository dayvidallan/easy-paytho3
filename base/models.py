...
class SolicitacaoLicitacao(models.Model):
    ...
    def get_valor_da_solicitacao(self):
        total = Decimal(0.00)
        propostas = ItemSolicitacaoLicitacao.objects.filter(solicitacao=self, eh_lote=False)
        for proposta in propostas:
...
class ItemSolicitacaoLicitacao(models.Model):
    ...
    def get_reducao_empresa(self):
        ...
        if lance_minimo:
            ...
            if preco and preco[0].valor and self.get_lance_minimo_valor():
...
class Pregao(models.Model):
    ...
    def eh_pregao_outras_modalidades(self):
        return self.modalidade.id in [ModalidadePregao.TOMADA_PRECO, ModalidadePregao.CONCORRENCIA, ModalidadePregao.CONCORRENCIA_SRP, ModalidadePregao.CARTA_CONVITE]
...
class FornecedorCRC(models.Model):
    ...
    natureza_juridica = models.CharField(u'Natureza Jurídica', max_length=500, null=True, blank=True)
    ...
    data_expedicao = models.DateField(u'Data de Expedição', null=True, blank=True)
