                        ...
                        if propostas.exists():
                            ...
                            for proposta in propostas:
                                if eh_maior_desconto:
                                    ...
                                    total_propostas = total_propostas + proposta.valor
                                else:
...
def gestao_pedidos(request, tipo_id):
    setor = request.user.pessoafisica.setor
    form = GestaoPedidoForm(request.GET or None)
    nome = u''
    if tipo_id == u'1':
        nome = u'Contratos'
    elif tipo_id == u'2':
...
def gestao_contratos(request, tipo_id):
    ...
    if request.user.groups.filter(name=u'Gerente'):
        ...
        if form.is_valid():
            if form.cleaned_data.get('info'):
                valor = form.cleaned_data.get('info')
                registros = registros.filter(Q(solicitacao__objeto__icontains=valor) | Q(numero__icontains=valor) | Q(solicitacao__processo__numero__icontains=valor) | Q(solicitacao__num_memorando__icontains=valor))
...
def informar_valor_final_itens_lote(request, lote_id, pregao_id):
    ...
    if request.user.has_perm('base.pode_cadastrar_pregao') and pregao.solicitacao.recebida_setor(request.user.pessoafisica.setor):
        ...
        itens = ItemLote.objects.filter(lote=lote).order_by('numero_item')
        ids_itens_do_lote = ItemLote.objects.filter(lote=lote).values_list('item', flat=True)
        vencedor = lote.get_empresa_vencedora()
...
def visualizar_contrato(request, solicitacao_id):
    ...
    eh_desconto = contrato.pregao and contrato.pregao.eh_maior_desconto()
...
def ata_sessao_credenciamento(request, pregao_id):
    ...
    if pregao.tem_resultado():
        ...
        p.add_run(u'Diante da regularidade frente às exigências de habilitação contidas no instrumento convocatório, a Comissão Permanente de Licitação declarou como CREDENCIADA(s) a(s) empresa(s):')
                        ...
                        for [agenda_data_inicio, agenda_data_fim] in [
                            ...
                            if agenda_data_inicio.year == ano and agenda_data_inicio.month == mes:
                                ...
                                descricao = u'<a href="/base/pregao/{}/" target="_blank">{} {} <strong>{}</strong> {}</a>'.format(
                                    solicitacao.id,  solicitacao.modalidade, solicitacao.num_pregao, horario, solicitacao.get_situacao())
...
def baixar_licitacoes_portal(request):
    ...
    if 'xls' in request.GET:
        ...
        file_path = os.path.join(settings.MEDIA_ROOT, 'upload/modelos/relatorio_gerencial_situacao_licitacoes.xls')
        rb = open_workbook(file_path,formatting_info=True)
        ...
        for item in pregoes:
            row_index = contador + 1
            w_sheet.write(row_index, 0, conta_item)
            w_sheet.write(row_index, 1, item.num_pregao)
            w_sheet.write(row_index, 2, item.solicitacao.num_memorando )
            w_sheet.write(row_index, 3, item.objeto)
            w_sheet.write(row_index, 4, item.modalidade.nome)
            w_sheet.write(row_index, 5, item.get_situacao())
            data = u'%s às %s ' % (item.data_abertura.strftime('%d/%m/%Y'), item.hora_abertura.strftime('%H:%M'))
            w_sheet.write(row_index, 6, data)
            w_sheet.write(row_index, 7, format_money(item.get_valor_total()))
...
def baixar_contratos_portal(request):
    ...
    if 'xls' in request.GET:
        ...
        for item in contratos:
            ...
            data = u'%s a %s ' % (item.data_inicio.strftime('%d/%m/%Y'), item.data_fim.strftime('%d/%m/%Y'))
            w_sheet.write(row_index, 7, data)
            fornecedor = u'%s (%s)' % (item.get_fornecedor().razao_social, item.get_fornecedor().cnpj)
            w_sheet.write(row_index, 8, fornecedor)
            contador += 1
            conta_item += 1
...
@login_required()
def pedidos_arp_secretarias(request, ata_id):
    ata = get_object_or_404(AtaRegistroPreco, pk=ata_id)
