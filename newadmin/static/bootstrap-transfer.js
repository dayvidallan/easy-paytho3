(function($){
    $.fn.bootstrapTransfer = function(options) {
        ...
        return this.each(function(){
            ...
            _this.$hidden_select = _this.find('select.hidden');
            ...
            if (settings.target_id != '') {
                _this.$target_select.attr('id', 'id_'+settings.target_id);
                _this.$hidden_select.attr('name', settings.target_id);
            }
            /* height */
            ...
            _this.update_lists = function(force_hilite_off) {
                ...
                $(_this.$target_select).find('option').each(function() {
                    options+= "<option selected value='"+this.value+"'>"+this.innerHTML+"</option>";
                });
                $(_this.$hidden_select)[0].innerHTML = options;
    ...
    $.fn.bootstrapTransfer.defaults = {
        'template':                                         
            '<select multiple="multiple" class="hidden" style="width: 300px;"></select>\
             <div class="row-fluid">\
                <div class="col-md-12" style="padding: 3px">\
                    <input type="text" class="filter-input"><i class="icon-search"></i>\
                </div>\
                <div class="col-md-6">\
                    <select multiple="multiple" class="filtered remaining" style="width: 300px; margin-left: -15px">\
                    </select>\
                    <a class="btn btn-primary selector-add" href="javascript:" class="selector-add">Adicionar</a><br/>\
                    <a href="javascript:" class="selector-chooseall">Escolher Todos</a>\
                </div>\
                <div class="col-md-6">\
                    <select multiple="multiple" class="filtered target" style="width: 300px">\
                    </select>\
                    <a class="btn btn-danger selector-remove" href="javascript:" class="selector-remove">Remover</a><br/>\
                    <a href="javascript:" class="selector-clearall">Remover Todos</a>\
                </div>\
            </div>\
            ',
        'height': '10em',
